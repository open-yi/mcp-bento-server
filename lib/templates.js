...
const TEMPLATES = {
  blank: {
    ...
    label: 'Blank',
    emoji: '⬜',
    description: 'One clean white slide, no content. The default — start from nothing.',
    theme: { background: '#FFFFFF', color: '#1A1D20', accent: '#2563EB', fontFamily: 'system-ui, sans-serif' },
...
function makeDoc(tplName, title) {
  const tpl = TEMPLATES[tplName] || TEMPLATES.blank;
  if (tpl.name === 'blank') {
    return {
      format: 'bento/slides',
      version: 1,
      ...
      theme: tpl.theme,
      slides: [{ id: 's1', background: '#FFFFFF', transition: 'none', notes: '', elements: [] }],
    };
  }
