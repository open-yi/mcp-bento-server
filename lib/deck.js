...
function applyPatch(doc, ops) {
  ...
  if (ops.addSlides) {
    for (const s of ops.addSlides) {
      ...
      // new content slides default to a white background so presenting never
      // shows black; the cover keeps its template theme color
      if (!s.background) s.background = '#FFFFFF';
      doc.slides.push(s);
    }
  }
