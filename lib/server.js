...
class BentoServer {
  ...
  patch(ops) {
    if (!this.doc) throw new Error('No deck open. Use open <file> or new first.');
    // Live-building discipline: force agents to work one element / one slide at
    // a time so the user always sees each step land. Batch adds are rejected
    // with a hint (the agent splits them into separate calls).
    if (ops.createElements && ops.createElements.length > 1) {
      throw new Error('One element at a time for live building — send createElements with a single element (repeat the call per element).');
    }
    if (ops.addSlides && ops.addSlides.length > 1) {
      throw new Error('One slide at a time for live building — send addSlides with a single slide.');
    }
    ...
    // which slide should the browser activate? agent can say, else infer from the ops
    ...
    if (hadSetDoc) {
      ...
      // typing effect: reveal updated text word/chunk by chunk (~120ms each)
      this._streamTextUpdates(ops.updateElements, gen);
    } else if (newElements.length > 1) {
      // element-by-element reveal so the user watches the page grow
      this._streamElementReveal(beforeDoc, newElements, gen);
    } else {
  ...
  // Reveal multiple new elements one at a time (~200ms each) so the user sees
  // the page being built, instead of a whole batch appearing at once.
  async _streamElementReveal(beforeDoc, newElements, gen, stepMs = 200) {
    const total = newElements.length;
    for (let i = 1; i <= total; i++) {
      ...
      this._broadcast('doc-updated', { file: this.deckFile, doc: partial });
