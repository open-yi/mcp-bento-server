...
(function () {
  ...
  // ── reverse sync: report manual browser edits back to the server ──────
  // The user can edit in the browser (drag, retype) while the agent works.
  // Poll the live document; when it differs from what we last applied
  // (agent pushes) and from what we last reported, it was a manual edit —
  // push it to the server so agent read/patch always see the latest.
  var lastReported = null;
  setInterval(function () {
    if (!window.bento || !window.bento.doc) return;
    var cur;
    try { cur = JSON.stringify(window.bento.doc); } catch (e) { return; }
    if (cur === lastApplied || cur === lastReported) return; // agent push or already reported
    lastReported = cur;
    fetch(ORIGIN + '/api/user-edit', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ doc: window.bento.doc })
    }).catch(function () {});
  }, 2000);
})();
