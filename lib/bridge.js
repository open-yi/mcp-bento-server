...
(function () {
  ...
  // Apply a new document without the "flash to slide 1" flicker:
  // hide the editor, loadDoc, restore the target slide, then reveal.
  function applyDoc(docStr, targetIndex) {
    var root = document.querySelector('.ed-root') || document.body;
    ...
    } catch (e) { root.style.opacity = prev || ''; return; }
    activateSlide(targetIndex, function () { root.style.opacity = prev || ''; });
  }
    ...
    if (window.bento && typeof window.bento.loadDoc === 'function' && msg.doc) {
      try {
        ...
        if (applied === cur) {
          ...
          post({ type: 'loaddoc-ok', slides: msg.doc.slides.length });
        }
        return;
      } catch (e) {
