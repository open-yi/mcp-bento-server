...
const TOOLS = [
  { name: 'bento_list_templates', description: "List built-in deck templates (name, style, when to use). Pick one for bento_new_deck based on the user's topic.", inputSchema: { type: 'object', properties: {} } },
  ...
  { name: 'bento_new_deck', description: 'Create a new deck file from a template (see bento_list_templates).', inputSchema: { type: 'object', properties: { file: { type: 'string' }, title: { type: 'string' }, template: { type: 'string', description: 'dark | light | gradient | editorial | midnight' } }, required: ['file'] } },
  ...
  { name: 'bento_present', description: 'Start the presentation: fullscreen=true (default) goes OS-fullscreen, false presents inside the tab.', inputSchema: { type: 'object', properties: { fullscreen: { type: 'boolean' } } } },
...
async function callTool(name, args) {
  ...
  switch (name) {
    ...
    case 'bento_new_deck': return resultOf(await api('POST', '/api/new', { file: args.file, title: args.title || 'Untitled deck', template: args.template }));
    case 'bento_list_templates': {
      const { templateSummary } = require('./templates');
      return resultOf(templateSummary());
    }
    ...
    case 'bento_present': return resultOf(await api('POST', '/api/present', { fullscreen: args.fullscreen !== false }));
